...
wifi5 \: optional
    ...
    be set up through `outdoor_chanlist`, which allows for a combination of single
    channels and channel ranges. When set this offers the outdoor mode flag for 5 GHz
    radios in the config mode which moves the AP to the configured outdoor chanlist and
    disables mesh on that radio.
    ::
